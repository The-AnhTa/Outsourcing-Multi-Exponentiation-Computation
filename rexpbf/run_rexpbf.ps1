...
if ($RunArguments.Count -ne 2 -or
    ...
    throw "Usage: .\run_rexpbf.ps1 --d <dimension>"
}

$d = 0
if (-not [int]::TryParse($RunArguments[1], [ref]$d) -or
    $d -lt 1 -or $d -gt 63) {
    throw "The dimension must be an integer from 1 through 63."
}

$projectRoot = $PSScriptRoot
$buildDirectory = Join-Path $projectRoot "build"
$executable = Join-Path $buildDirectory "run_rexpbf.exe"
...
$nativeCommand =
    "call `"$vcvars`" >nul && " +
    $configureCommand +
    "cmake --build `"$buildDirectory`" --target run_rexpbf -j 4 >nul"
...
if ($buildExitCode -ne 0) {
    throw "The run_rexpbf build failed: $($buildOutput -join [Environment]::NewLine)"
}

$output = & $executable $d 2>&1
if ($LASTEXITCODE -ne 0) {
    throw "Rexp.bf failed."
}
