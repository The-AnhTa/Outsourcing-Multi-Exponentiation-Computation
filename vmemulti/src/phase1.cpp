...
Phase1Result prove_phase1(const VmeIbfCRS& crs,
    ...
    const G2 output = g2_multiexp_protocol(crs.H, input.x);
    const Digest digest = compute_statement_digest(crs, input, output);
    auto result = prove_phase1_core(
        crs, precomputation, input.x, output, Transcript(digest));
    result.statement.digest = digest;
    return result;
}

Phase1Result prove_phase1_core(const VmeIbfCRS& crs,
                              const VmeIbfPrecomputation& precomputation,
                              std::span<const Fr> aggregate_exponents,
                              const G2& aggregate_output,
                              Transcript transcript) {
    if (!validate_crs(crs)
        || !audit_precomputation(crs, precomputation)
        || aggregate_exponents.size() != crs.n
        || !valid_g2(aggregate_output))
        throw std::invalid_argument("invalid aggregate phase-1 input");
    Phase1Result result;
    result.transcript_start = transcript.digest();
    result.statement.x.assign(
        aggregate_exponents.begin(), aggregate_exponents.end());
    result.statement.X = aggregate_output;
    VmeIbfStatementInput statement_input{result.statement.x, {}};
    statement_input.digest = compute_statement_input_digest(
        crs, statement_input.x);
    result.statement.digest = compute_statement_digest(
        crs, statement_input, result.statement.X);
