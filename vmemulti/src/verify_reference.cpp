...
ReferenceVerificationTrace verify_reference_impl(
    ...
    try {
        ...
        if (!internal::replay_protocol(
                crs, precomputation, statement, proof, challenges,
...
ReferenceVerificationTrace verify_reference_diagnostic(
    const VmeIbfCRS& crs, const VmeIbfPrecomputation& precomputation,
    const VmeIbfStatement& statement, const VmeIbfProof& proof) {
    return verify_reference_impl(
        crs, precomputation, statement, proof, nullptr);
}
...
ReferenceVerificationTrace verify_reference_core_diagnostic(
    ...
    const Digest state = transcript.digest();
    return verify_reference_impl(
        crs, precomputation, statement, proof, &state);
}
