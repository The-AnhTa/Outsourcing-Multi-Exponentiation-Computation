...
ReferenceVerificationTrace verify_reference_core_diagnostic(const VmeIbfCRS&c,const VmeIbfPrecomputation&p,const VmeIbfStatement&s,const VmeIbfProof&proof,Transcript T){
 ReferenceVerificationTrace tr;try{if(!valid_inputs(c,p,s,proof))return tr;tr.rho.resize(c.d);std::vector<DoryTargetState>fresh(c.d+1);GT outer=p.pairing_x[0];
...
ReferenceVerificationTrace verify_reference_diagnostic(const VmeIbfCRS&c,const VmeIbfPrecomputation&p,const VmeIbfStatement&s,const VmeIbfProof&proof){return verify_reference_core_diagnostic(c,p,s,proof,Transcript(s.digest));}
