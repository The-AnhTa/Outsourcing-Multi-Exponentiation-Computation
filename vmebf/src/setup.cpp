...
SetupResult setup(std::size_t d,const std::vector<G2>&H,ScalarRng&rng){
 initialize();if(d<1||d>=std::numeric_limits<std::size_t>::digits)throw std::invalid_argument("invalid d");size_t n=size_t{1}<<d;if(H.size()!=n)throw std::invalid_argument("public_H length mismatch");for(auto&p:H)if(!valid_g2(p,true))throw std::invalid_argument("invalid public H");
 ...
 o.statement_input.x.reserve(n);for(size_t i=0;i<n;++i)o.statement_input.x.push_back(rng.random_fr());o.statement_input.digest=compute_statement_input_digest(o.crs,o.statement_input.x);o.prover_input.x=o.statement_input.x;return o;
}
}
