...
if ($RunArguments.Count -ne 2 -or
    ...
    throw "Usage: .\run_vme.ps1 --d <dimension>"
}
...
$executable = Join-Path $buildDirectory "run_vme.exe"
...
$nativeCommand =
    "call `"$vcvars`" >nul && " +
    $configureCommand +
    "cmake --build `"$buildDirectory`" --target run_vme -j 4 >nul"
...
if ($buildExitCode -ne 0) {
    throw "The run_vme build failed: $($buildOutput -join [Environment]::NewLine)"
}
