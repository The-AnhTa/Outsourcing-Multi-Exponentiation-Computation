...
if ($RunArguments.Count -ne 4) {
    throw "Usage: .\run_pippenger.ps1 --d <dimension> --k <instances>"
}
...
$executable = Join-Path $buildDirectory "run_pippenger.exe"
...
$nativeCommand =
    "call `"$vcvars`" >nul && " +
    $configureCommand +
    "cmake --build `"$buildDirectory`" --target run_pippenger -j 4 >nul"
...
if ($buildExitCode -ne 0) {
    throw "The run_pippenger build failed: $($buildOutput -join [Environment]::NewLine)"
}

$output = & $executable $d $k 2>&1
if ($LASTEXITCODE -ne 0) {
    throw "Multi-instance Pippenger failed."
}
