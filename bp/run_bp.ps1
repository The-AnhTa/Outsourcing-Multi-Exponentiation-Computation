...
if ($RunArguments.Count -ne 2 -or
    ...
    throw "Usage: .\run_bp.ps1 --d <dimension>"
}
...
$executable = Join-Path $buildDirectory "run_bp.exe"
...
$nativeCommand =
    "call `"$vcvars`" >nul && " +
    $configureCommand +
    "cmake --build `"$buildDirectory`" --target run_bp -j 4 >nul"
...
if ($buildExitCode -ne 0) {
    throw "The run_bp build failed: $($buildOutput -join [Environment]::NewLine)"
}
