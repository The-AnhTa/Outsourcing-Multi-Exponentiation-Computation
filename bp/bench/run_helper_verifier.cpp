...
std::size_t crs_payload_bytes(const HvPublicParams& parameters) {
...
int main(int argc, char** argv) {
  try {
    ...
    const std::string seed_text =
        "helper-verifier-single-run/d=" + std::to_string(d);
    const Bytes seed(seed_text.begin(), seed_text.end());
    const HvSetupResult setup = setup_helper_verifier(n, seed);
    const HvInstance instance =
        generate_helper_verifier_instance(setup.pp);

    const auto proof =
        prove_helper_verifier(setup.pp, setup.helper_precomp,
                              instance.statement);
    if (!proof)
      throw std::runtime_error("helper-verifier proving failed");
    const Bytes wire = serialize_hv_proof(setup.pp, *proof);

    HvVerifyTimings timings;
    if (!verify_helper_verifier(setup.pp, setup.verifier_precomp,
                                instance.statement, *proof, &timings))
      throw std::runtime_error("helper-verifier verification failed");

    const double verifier_ms =
        timings.total_ms - timings.validation_ms;
    const std::size_t crs_size = crs_payload_bytes(setup.pp);

    std::cout << std::fixed << std::setprecision(3)
              << "Verification time: " << verifier_ms << " ms\n"
