...
if ($RunArguments.Count -ne 2 -or
    ...
    throw "Usage: .\run_helper_prover.ps1 --d <dimension>"
}
...
$executable = Join-Path $buildDirectory "run_helper_prover.exe"
...
$nativeCommand =
    "call `"$vcvars`" >nul && " +
    $configureCommand +
    "cmake --build `"$buildDirectory`" --target run_helper_prover -j 4 >nul"
...
if ($buildExitCode -ne 0) {
    throw "The helper-prover build failed: $($buildOutput -join [Environment]::NewLine)"
}

$output = & $executable $d 2>&1
if ($LASTEXITCODE -ne 0) {
    throw "The helper-prover protocol failed."
}
