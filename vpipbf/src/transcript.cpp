...
Transcript::Transcript(std::span<const std::uint8_t> d){Bytes b;append_frame(b,"vpipbf/fs/v1");append_frame(b,"BN254/mcl-v3.00");append_frame(b,d);state_=sha256(b);}
Transcript Transcript::resume(const Digest& state){return Transcript(state,ResumeTag{});}
void Transcript::absorb(std::string_view label,std::span<const Bytes> fields){Bytes b;append_frame(b,"vpipbf/fs/v1/absorb");append_frame(b,label);append_frame(b,state_);for(const auto&f:fields)append_frame(b,f);state_=sha256(b);}
