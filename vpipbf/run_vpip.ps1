...
if ($RunArguments.Count -ne 2 -or
    ...
    throw "Usage: .\run_vpip.ps1 --d <dimension>"
}
...
$executable = Join-Path $buildDirectory "run_vpip.exe"
...
$nativeCommand =
    "call `"$vcvars`" >nul && " +
    $configureCommand +
    "cmake --build `"$buildDirectory`" --target run_vpip -j 4 >nul"
...
if ($buildExitCode -ne 0) {
    throw "The run_vpip build failed: $($buildOutput -join [Environment]::NewLine)"
}
