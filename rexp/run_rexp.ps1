...
if ($RunArguments.Count -ne 2 -or
    ...
    throw "Usage: .\run_rexp.ps1 --d <dimension>"
}
...
$executable = Join-Path $buildDirectory "run_rexp.exe"
...
$nativeCommand =
    "call `"$vcvars`" >nul && " +
    $configureCommand +
    "cmake --build `"$buildDirectory`" --target run_rexp -j 4 >nul"
...
if ($buildExitCode -ne 0) {
    throw "The run_rexp build failed: $($buildOutput -join [Environment]::NewLine)"
}
